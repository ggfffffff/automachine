...
public:
    ...
    std::vector<int> group_begin; // 组开头
    std::vector<int> group_end; // 组结尾

    //若有区间限定，则存对应epsilon转移的索引，没有则-1
    std::vector<int> rangestate;
     ...
     */
    Path exec(std::string text, bool is_start);
    bool special_match(char by, char c);
    bool fun(int q, std::string str, int step, std::vector<int> e_stat,  bool is_start);
